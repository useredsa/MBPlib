...
void HistoryUpdate(uint64_t PC, const mbp::Branch& opType, bool taken,
                   uint64_t target, long long& X, int& Y, folded_history* H,
                   folded_history* G, folded_history* J) {
  int brtype = (opType.isIndirect() ? 2 : 0) + (opType.isConditional() ? 1 : 0);
...
void UpdatePredictor(uint64_t PC, const mbp::Branch& opType, bool resolveDir,
...
void TrackOtherInst(uint64_t PC, const mbp::Branch& opType, bool taken,
                    uint64_t branchTarget) {
  HistoryUpdate(PC, opType, taken, branchTarget, phist, ptghist, ch_i, ch_t[0],
                ch_t[1]);
}
